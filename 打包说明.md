...
3. **重要：** 确保本地的 `.github/workflows/build.yml` 文件已提交并推送到 GitHub。
   ```bash
   git add .
   ...
   **或者（如果不会用命令行）：**
   ...
   2. 文件名输入：`.github/workflows/build.yml` (注意开头的点和路径)。
   3. 将本地同名文件的内容复制粘贴进去。
   4. 点击 "Commit changes"。

4. 进入仓库的 "Actions" 标签页。
5. **不要搜索！** 直接看页面 **左侧列表**，会出现 "Build Windows App" 选项。
6. 点击它，然后在右侧点击 "Run workflow" 按钮。
7. 等待构建完成后，在 Artifacts 区域下载 `MaterialReview_Windows.zip`。
